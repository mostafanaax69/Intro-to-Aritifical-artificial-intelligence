...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        time_limit = max(self.game_time*0.5,3)
        self.game_time=0.5*self.game_time
        ...
        while passedTime + next_iteration_max_time < time_limit:
            ...
            (val, move,_d) = alpha_beta_alg.search((move, self.loc, self.board, self.rival_loc, fruit_list), depth, True)
    ...
    def builtIn_heuristic(self, state):
        ...
        if minDist_agent_rival > 0:
            if free_rivalAttainable_spots > 0 and flag_fruitIsReachable:
                res = (free_agentAttainable_spotsSum / max(minDist_agent_rival, free_rivalAttainable_spots))
                return res
